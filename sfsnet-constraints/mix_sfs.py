...
from datasets import PseudoDataset, SfSNetDataset
...
parser.add_argument('--syn-data-dir', type=str)
...
tl(f'Mix sfsnet: {batch_size}, no of epochs: {num_epochs}, data: {args.pseudo_data_dir}, out_dir: {args.out_dir}')
print(args)
tl(f'lr: {args.lr} with mse loss')

# synthetic bfm dataset
full_dataset = SfSNetDataset(args.syn_data_dir)
...
syn_train_dataset, syn_val_dataset = random_split(
    full_dataset, [train_count, validation_count])

# pseudo bfm dataset
full_dataset = PseudoDataset(args.pseudo_data_dir)
dataset_size = len(full_dataset)
validation_split = args.val_size
validation_count = int(validation_split * dataset_size / 100)
train_count = dataset_size - validation_count

psuedo_train_dataset, psuedo_val_dataset = random_split(
    full_dataset, [train_count, validation_count])

tl((100 - validation_split))

# now mix dataset
train_dataset = ConcatDataset([syn_train_dataset, psuedo_train_dataset])
val_dataset = ConcatDataset([syn_val_dataset, psuedo_val_dataset])
mix_train_dl  = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=8)
...
if args.use_pretrained == 'yes':
    sfs_net.load_state_dict(torch.load(args.model, map_location=device))
else:
...
lamda_normal = 0.5
lamda_sh = 0.1
mix_train_len = len(mix_train_dl)
loss_dict = {
    'total': [],
    'albedo': [],
    'normal': [],
...
for epoch in range(1, num_epochs+1):
    ...
    for bix, data in enumerate(mix_train_dl):
    ...
    cur_tloss = tloss / mix_train_len
    cur_nloss = nloss / mix_train_len
    cur_aloss = aloss / mix_train_len
    cur_shloss = shloss / mix_train_len
    cur_rloss = rloss / mix_train_len
    ...
    tl(f'Sfsnet: Training set results Data path {args.pseudo_data_dir}: Total Loss: {cur_tloss}, \
    Normal Loss: {cur_nloss}, Albedo Loss: {cur_aloss}, SH Loss: {cur_shloss}, \
    Recon Loss: {cur_rloss}')
...
plt.legend(['Total Loss', 'Albedo Loss', 'Normal Loss',
            'Shading Loss', 'Reconstruction Loss'])
plt.savefig(f'{args.out_dir}/result_sfs.png')


torch.save(sfs_net.state_dict(), f'{args.out_dir}/mix-sfsnet.pth')

tl(f'training done bfm {args.pseudo_data_dir}')
