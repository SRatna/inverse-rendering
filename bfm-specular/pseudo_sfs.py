...
tl(f'sfsnet: {batch_size}, no of epochs: {num_epochs}, data: {args.data_path}, out_dir: {args.out_dir}')
tl('using bfm dataset')
print(args)
tl(f'lr: {args.lr} with mse loss')
full_dataset = PseudoDataset(args.data_path)
...
for epoch in range(1, num_epochs+1):
    ...
    tl(f'Sfsnet: Training set results Data path {args.data_path}: Total Loss: {cur_tloss}, \
    Normal Loss: {cur_nloss}, Albedo Loss: {cur_aloss}, SH Loss: {cur_shloss}, \
    Specular Loss: {cur_sloss}, Recon Loss: {cur_rloss}')

    save_image(face, f'tmp/face_{epoch}.png')
    save_image(normal, f'tmp/normal_{epoch}.png', True)
    save_image(predicted_normal, f'tmp/predicted_normal_{epoch}.png', True)
    save_image(albedo, f'tmp/albedo_{epoch}.png')
    save_image(predicted_albedo, f'tmp/predicted_albedo_{epoch}.png')
    save_image(out_shading, f'tmp/out_shading_{epoch}.png')
    save_image(out_recon, f'tmp/out_recon_{epoch}.png')
...
torch.save(sfs_net.state_dict(), f'{args.out_dir}/pseudo-sfsnet-bfm.pth')

tl(f'training done bfm {args.data_path}')
