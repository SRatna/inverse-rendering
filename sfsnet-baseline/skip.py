...
from datasets import SfSNetDataset
...
tl(f'skipnet: {args.data_dir}, {batch_size}, no of epochs: {num_epochs}, out dir: {args.out_dir}')
print(args)
...
if args.use_pretrained == 'yes':
    skip_net.load_state_dict(torch.load(args.model))
else:
    skip_net.apply(weights_init)
skip_net = skip_net.to(device)
model_parameters = skip_net.parameters()
...
for epoch in range(1, num_epochs+1):
    ...
    for bix, data in enumerate(syn_train_dl):
        ...
        face = face * mask
        predicted_normal, predicted_albedo, predicted_sh, out_shading, out_recon = skip_net(
            face)
    ...
    tl(f'Skipnet: Training set results Data path {args.out_dir}: Total Loss: {cur_tloss}, \
...
plt.savefig(f'{args.out_dir}/result_skipnet.png')


torch.save(skip_net.state_dict(), f'{args.model}')
...
with torch.no_grad():
    ...
    for bix, data in enumerate(syn_val_dl):
        ...
        face = face.to(device)
        face = face * mask
        predicted_normal, predicted_albedo, predicted_sh, predicted_shading, predicted_face = skip_net(
            face)
        save_image(face, f'{args.out_dir}/face_skipnet.png')
        save_image(predicted_normal, f'{args.out_dir}/normal_skipnet.png', True)
        save_image(predicted_albedo, f'{args.out_dir}/albedo_skipnet.png')
        save_image(predicted_shading, f'{args.out_dir}/shading_skipnet.png')
        save_image(predicted_face, f'{args.out_dir}/recon_skipnet.png')
        i += 1
        if i > 1:
            break
